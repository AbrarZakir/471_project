...
export default function UserProfilePage() {
  ...
  // 1) Once auth guard is done, fetch session & profile row
  useEffect(() => {
    if (loading) return
    ...
    supabase.auth.getSession().then(({ data }) => {
      ...
      // a) Pre-fill email from auth
      setProfile(prev => ({ ...prev, email: user.email! }))

      // b) Load extra fields from profiles table
      supabase
        .from('profiles')
  ...
  async function handleSave() {
    ...
    const { error: updateAuthError } = await supabase.auth.updateUser({
      email:    profile.email,
      password: password || undefined,  // only if non-empty
    })
    if (updateAuthError) {
